...
def hDispersion(X, y):
    ...
    return np.sqrt(np.sqrt(np.sum(np.linalg.eigvals(cov)**2)))
...
def iDistribution(X, y, l = 0.5):
    ...
    return (np.sqrt(((W - w_hat)**2).sum(axis = 1)) < l).mean()



def aDistribution(X, y, alpha = 0.1):
    statmodel = define_model(y)
...
def klFunction(X, y):
    ...
    indexes = np.random.permutation(X.shape[0])[:100]
