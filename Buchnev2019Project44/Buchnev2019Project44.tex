...
Предполагается, что выборка $\mathfrak D$ не противоречит гипотезе порождения данных: модель, порождающая данные, задается в следующем виде:
...
$$
p(y | \mathbf{x}, \mathbf{w}) \sim \mathcal{N}(f(\mathbf{x}, \mathbf{w}), S(\mathbf{w}^{\top}\mathbf{x})),
$$
где $S(\mathbf{w}^{\top}\mathbf{x})$ --- выборочная дисперсия выборки $\{\mathbf{w}^{\top}\mathbf{x}_i\}_{i=1}^{m}$.

Для модели логистической регрессии зависимая переменная порождается бернуллиевским распределением:
$$
...
Для произвольного множества $\mathcal{A} = \{a_1, ..., a_k\} \subset \mathbb{J} = \{1, 2, ..., n\}$, определим:
$$
...
Для оптимального набора параметров и оценки вектора параметров используется принцип максимума правдоподобия:
...
$$
l(m_0) = \frac{1}{m_0}\underset{\mathfrak D^{\prime} \subset \mathfrak D, |\mathfrak D^{\prime}| = m_0}{\mathsf E} l(\hat{\mathbf{w}} | \mathfrak D^{\prime}).
$$
...
\begin{enumerate}
	\item Генерируется случайный вектор индексов $\mathbf{i} = (i_1, ..., i_{m_0})$, где:
	\begin{itemize}
...
$$
\hat{l}_{\mathbf{i}}(m_0) = \frac{1}{m_0}l(\hat{\mathbf{w}} | \mathfrak D_{\mathbf{i}})
$$
	\item пп. 1-2 повторяются $K$ раз, оценка $\hat{l}(m_0)$ функции $l(m_0)$ задается формулой:
$$
\hat{l}(m_0) = \frac{1}{K}\sum\limits_{k=1}^K \hat{l}_{\mathbf{i}_k}(m_0),
$$
где $\mathbf{i}_k$ --- вектор индексов, сгенерированный на итерации $k$.
\end{enumerate}
...
Оценка вектора параметров $\hat{\mathbf{w}}$ является решением задач (\ref{argmax_L}), (\ref{argmin_MSE}):
...
\begin{enumerate}
	...
	\item Вычисляется оценка вектора параметров $\hat{\mathbf{w}}$ как решение оптимизационной задачи (\ref{argmin_MSE}) на выборке $\mathfrak D_{\mathcal{L}}$.
...
На рис. 6 построена зависимость среднего значения элементов вектора параметров $\hat{\mathbf{w}}$ от величины $\tau$ при обучении Лассо регрессии (\ref{argmin_MSE}), (\ref{MSE_lasso}) на подвыборках размера $m_0 = 100$.

На рис. 7, 8 построены зависимости матожидания и дисперсии функции $e^{-MSE(\mathbf{x}, y, \hat{\mathbf{w}})}$ от объёма выборки и от количества используемых признаков.
...
На рис. 9 построена зависимость значения элементов оценки ковариационной матрицы параметров $\hat{A}$ от количества используемых признаков $n^{\prime}$ при обучении модели линейной регрессии (\ref{argmin_MSE}), (\ref{MSE}). Можно заметить, что ковариация между парой признаков $\mathbf{w}_i, \mathbf{w}_j$, попавших в набор используемых признаков $\mathcal{A}_{n^{\prime}}$, слабо зависит от $n^{\prime}$ и остается практически неизменной.

На рис. 10 построена зависимость значения элементов оценки ковариационной матрицы параметров $\hat{A}$ от коэффициента регуляризации $\tau$ при обучении Лассо регрессии (\ref{argmin_MSE}), (\ref{MSE_lasso}). Можно заметить, что ковариация $cov(\mathbf{w}_i, \mathbf{w}_j)$ обратно пропорциональна величине $\tau$.
