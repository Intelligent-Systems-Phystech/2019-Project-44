...
Рассмотрим критерий средней длины. Радиус доверительного шара аппроксимируется следующим образом: используется предположение о распределении оценки вектора параметров:

$$
\hat{\textbf{w}} \sim \mathcal{N}(\textbf{m}, \textbf{V}),~~~ \textbf{V} = \textbf{I}^{-1}(\mathfrak D, m),
$$
далее с помощью сэмплирования вычисляется приближенное значение $r_m$.
...
Рассмотрим матрицу $\hat{\textbf{I}}(\mathfrak{D}, m)^{-1}$  как аппроксимацию ковариационной матрицы вектора параметров для $m$ наблюдений и вычислим $r_m$.

Таким образом, построена аппроксимация функции эффективности для критерия средней длины.
