...
S_{reg}(\mathbf{x}, y, \textbf{w}) = \frac{1}{m}\sum\limits_{i=1}^{m}(y_i - \textbf{w}^{\top}\mathbf{x}_i )^2, ~~~ S_{class} = \sum\limits_{i=1}^{m}(y_i\ln f(\mathbf{x}_i, \mathbf{w}) + (1 - y_i)\ln(1 - f(\mathbf{x}_i, \mathbf{w}))).
\end{equation}
...
\begin{enumerate}
	\item Из равномерного распределения генерируется случайный вектор индексов $\mathbf{i} = (i_1, ..., i_{m_0})$, где:
	\begin{itemize}
		\item $i_j \sim U(\{1, ..., m_0\})$ --- эксплутационный вариант.
...
\subsection{Аппроксимация $\hat{l}(m)$}

На рис. 2 представлена функция $\hat{l}(m)$, посчитанная в двух вариантах: практическом и теоретическом. Для теоретического способа сходимость зависит только от объёма выборки $m$, а для практического сходимость зависит еще и от доступных элементов выборки $\mathfrak{D}$.

\begin{figure}[h!t]\center
\includegraphics[width=1\textwidth]{../data/pics/synthetic_l_comparison.pdf}
\caption{Зависимость значения функции $\hat{l}$ от объема выборки}
\label{fig1.5}
\end{figure}

На рис. 3 представлена зависимость функции ошибки MAPE от объёма выборки. В теоретическом варианте MAPE сильно меньше, так как функция $\hat{l}(m)$ лучше аппроксимируется семейством функций $\Phi$.

\begin{figure}[h!t]\center
\includegraphics[width=1\textwidth]{../data/pics/synthetic_l_MAPE_comparison.pdf}
\caption{Зависимость значения функции ошибки MAPE от объема выборки}
\label{fig1.7}
\end{figure}

\begin{figure}[h!t]\center
\includegraphics[width=1\textwidth]{../data/pics/synthetic_l_MAPE_cheat_examples.pdf}
\caption{Примеры аппркосимации функции $\hat{l}(m)$, теоретический способ}
\label{fig1.8}
\end{figure}

\begin{figure}[h!t]\center
\includegraphics[width=1\textwidth]{../data/pics/synthetic_l_MAPE_honest_examples.pdf}
\caption{Примеры аппркосимации функции $\hat{l}(m)$, практический способ}
\label{fig1.9}
\end{figure}
...
На рис. 6 представлена зависимость элементов ковариационной матрицы вектора параметров от объема выборки. [Комментарий?].
Зависимость посчитана с помощью метода бутстреп:
\begin{enumerate}
...
На рис. 7 представлена зависимость элементов матрицы Фишера от объема выборки. Оценка информационной матрицы Фишера $\hat{I}$ выражена через оценку ковариационной матрицы по формуле (\ref{w_hat_I}).
...
На рис. 8 построена зависимость среднего значения элементов вектора параметров $\hat{\mathbf{w}}$ от величины $\tau$ при обучении Лассо регрессии (\ref{argmax_L}), (\ref{MSE_lasso}) на подвыборках размера $m_0 = 100$.

На рис. 9, 10 построены зависимости матожидания и дисперсии функции $e^{-MSE(\mathbf{x}, y, \hat{\mathbf{w}})}$ от объёма выборки и от количества используемых признаков.
...
На рис. 11 построена зависимость значения элементов оценки ковариационной матрицы параметров $\hat{A}$ от количества используемых признаков $n^{\prime}$ при обучении модели линейной регрессии (\ref{argmax_L}), (\ref{MSE}). Можно заметить, что ковариация между парой признаков $\mathbf{w}_i, \mathbf{w}_j$, попавших в набор используемых признаков $\mathcal{A}_{n^{\prime}}$, слабо зависит от $n^{\prime}$ и остается практически неизменной.

На рис. 12 построена зависимость значения элементов оценки ковариационной матрицы параметров $\hat{A}$ от коэффициента регуляризации $\tau$ при обучении Лассо регрессии (\ref{argmax_MSE}), (\ref{MSE_lasso}). Можно заметить, что ковариация $cov(\mathbf{w}_i, \mathbf{w}_j)$ обратно пропорциональна величине $\tau$.
